...
export function StepHeader({ locale, titleKey, subtitleKey, showExpertTag }: StepHeaderProps) {
  ...
  return (
    <div className="border-b border-zinc-200 dark:border-zinc-800 pb-4 mb-6">
      {showExpertTag && (
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 mb-3 bg-orange-600/10 dark:bg-orange-600/20 rounded-full">
          <span className="w-1.5 h-1.5 bg-orange-600 rounded-full" />
          <span className="text-orange-600 dark:text-orange-500 font-semibold text-[10px] uppercase tracking-wide">
            {toolTranslations.expertTag}
          </span>
        </div>
      )}

      <h2 className="text-2xl sm:text-3xl font-bold text-zinc-950 dark:text-zinc-50 mb-2 tracking-tight">
